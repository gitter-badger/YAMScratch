/*  Names: Dustin Hoffman, Alex Malin, Isaiah Bell, Shenghao Lu
    ...
    Date: 10-30-2014
    File name: Lab 6
    Program description: PD Control of Blimp
*/
...
// Global variables
unsigned char Data[2];          // used to get data from i2c
unsigned int range = 0;         //stores the ranger value
unsigned int motor_pw = 0;      //temp motor PW
...
void calibrate_steering(void)
{
    char *text;
    char i;
    int b[3] = {2765,2020,2585};    //all the values to the center, we can adjust
    ...
    for (i=1; i<3;i++)
    {
        ...
        while(1)        {
            ...
            input = getchar();
            if (input == 'u'){
                b[i] = b[i] - 10; //turning up is decrementing
                PCA0CP0 = 0xFFFF - b[i];
            }
            else if(input == 'd'){
                b[i] = b[i] + 10; //turn down is incrementing
                PCA0CP0 = 0xFFFF - b[i];
            }
            else if (input == 'q'){
                break;
