...
while xy_y_list(end) <= B-2
	[xy_x_list,xy_y_list,xy_z_list] = make_step(xy_x_list,xy_y_list,xy_z_list,A,B,C);
...
while xz_z_list(end) <= C-2
	[xz_x_list,xz_z_list,xz_y_list] = make_step(xz_x_list,xz_z_list,xz_y_list,A,C,B);
	%there may still be some x error
end
%burn down any remaining x coordinate
...
while yz_z_list(end) <= C-2
	[yz_y_list,yz_z_list,yz_x_list] = make_step(yz_y_list,yz_z_list,yz_x_list,B,C,A);
	%there may still be some x error
end
%burn down any remaining x coordinate
...
for z = [z_offset:2:C-1]
	x_index = find(xz_z_list==z);
	level(end+1,1) = xz_x_list(x_index(1))+1;
	%find the largest y value 
	y_index = find(yz_z_list==z);
	level(end,2) = yz_y_list(y_index(1))+1;
	z_values(end+1) = z;
end
